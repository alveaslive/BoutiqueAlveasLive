// Reçoit une action d'échange du navigateur (publier / accepter / annuler),
// vérifie l'identité Twitch (cookie signé), puis dépose l'action dans Firebase
// pour que le BOT la valide (le bot est la source de vérité). Accessible via POST .../trade
export async function onRequest(context) {
  ...
  // attendre le verdict du bot
  const resPath = `${dbUrl}/pokebot_trades/${channel}/results/${actId}.json`;
  for (let i = 0; i < 20; i++) {
    await sleep(500);
    ...
    if (r.ok) {
      ...
      if (data) {
        ...
        return json({ ok: !!data.ok, message: data.message || "", error: data.ok ? undefined : (data.message || "Refusé"), offerId: data.offerId });
      }
    }
  }
  fetch(actPath, { method: "DELETE" }).catch(() => {});
  return json({ ok: false, error: "Le bot n'a pas répondu. Il doit être allumé pour les échanges." }, 504);
}
