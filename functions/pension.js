// Reçoit une action de pension du navigateur (soin de l'œuf / éclosion),
// vérifie l'identité Twitch (cookie signé), puis dépose l'action dans Firebase
// pour que le BOT la valide (source de vérité). Accessible via POST .../pension
export async function onRequest(context) {
  ...
  if (!["heat", "humid", "affection", "hatch", "status"].includes(type)) return json({ ok: false, error: "Action inconnue" }, 400);
  ...
  if (type === "heat") { action.value = Math.max(0, Math.min(100, Number(body.value) || 0)); }

  const actId = "act_" + Date.now() + "_" + Math.random().toString(36).slice(2, 8);
  const actPath = `${dbUrl}/pokebot_pension/${channel}/actions/${actId}.json`;
  ...
  const resPath = `${dbUrl}/pokebot_pension/${channel}/results/${actId}.json`;
  for (let i = 0; i < 20; i++) {
    await sleep(500);
    ...
    if (r.ok) {
      ...
      if (data) {
        ...
        return json({ ok: !!data.ok, message: data.message || "", error: data.ok ? undefined : (data.message || "Refusé"), status: data.status, espece: data.espece, shiny: data.shiny });
      }
    }
  }
  fetch(actPath, { method: "DELETE" }).catch(() => {});
  return json({ ok: false, error: "Le bot n'a pas répondu. Il doit être allumé pour prendre soin de l'œuf." }, 504);
}
